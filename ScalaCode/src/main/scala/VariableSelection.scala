...
/**
  ...
  * Main + interaction effects are estimated as fixed effects using sum to 0 constraint to solve the identifiability problem.
  */
    ...
    for (i <- 1 until noOfIter) {
      ...
      for (j <- 0 until nj) {
        for (k <- 0 until nk) {
          ...
          // Estimate the prob for when the indicator=0
          val prob= (1-p)/(1-p + p*exp(tau*curTheta(j,k)*(SXjk-Njk*(mu+curAlpha(j)+curBeta(k)+0.5*pow(curTheta(j,k),2)))))

          val u= breeze.stats.distributions.Uniform(0,1).draw()
          if(prob<u) {     // the prob estimated above is for when the indicator=0, so if the prob is < u the indicator should be 1
